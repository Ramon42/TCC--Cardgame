...
function scr_calculate_dmg(_atk_id, _def_id, _sock, _sock_op, _bloq, _dmg){
	...
	for (var i = 0; i < array_length(con_server.instances_server); i++){
		if(con_server.instances_server[i, 2].inst_id == _atk_id) { 
			_atk = con_server.instances_server[i, 2]; 
		}
		else if (con_server.instances_server[i, 2].inst_id == _def_id) {
			_def = con_server.instances_server[i, 2]; 
		}
	}
